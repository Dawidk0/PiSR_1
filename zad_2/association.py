...
class MovieRecommendation:
    ...
    def load_data(self, folder="dataset_100k"):
        # Setting path
        data_folder = os.path.join(os.path.curdir, folder)
        rating_filename = os.path.join(data_folder, "ratings.csv")
        ...
        self.movies = pd.read_csv(
            os.path.join(movie_name_filename),
            usecols=['movieId', 'title'],
            dtype={'movieId': 'int32', 'title': 'str'})

        self.ratings_full = pd.read_csv(
            os.path.join(rating_filename),
            usecols=['userId', 'movieId', 'rating'],
            dtype={'userId': 'int32', 'movieId': 'int32', 'rating': 'float32'})

        self.movies.columns = ['MovieID', 'Title']
        self.ratings_full.columns = ['UserID', 'MovieID', 'Rating']
        print("loaded")
    ...
    def evaluate_model(self, rule_count=10):
        ...
        test_confidence = {}
        for candidate_rule in candidate_rules:
            tmp = float(valid_rule[candidate_rule] + invalid_rule[candidate_rule])
            if tmp > 0:
                test_confidence[candidate_rule] = valid_rule[candidate_rule]/tmp
            else:
                test_confidence[candidate_rule] = 0.0
        #test_confidence = 
        ...
        sorted_confidence = sorted(self.significant_rules.items(), key=itemgetter(1), reverse=True)
        self.test_confidencs = []
        self.test_confidence = test_confidence
        for x in sorted_confidence:
            self.test_confidencs.append(x[1])
        for index in range(rule_count):
            (premise, conclusion) = sorted_confidence[index][0]
            premise_names = "\n ".join(self.get_movie_name(movie_id=mov_id) for mov_id in premise)
            conclusion_name = self.get_movie_name(movie_id=conclusion)
...
if __name__ == '__main__':
    ...
    engine.data_prep(user_threshold=220)

    # Constructing itemsets
    engine.create_freq_itemsets(superset_max_size=10)
